 ...
 *   */15 * * * * node /path/to/farm-notifier.js >> /var/log/farm-notifier.log 2>&1
 *
 * =============================================================================
 */
...
async function notifyHarvestReady() {
  ...
  const { rows } = await pool.query(`
    SELECT
      ...
      ARRAY_AGG(DISTINCT p."cropType") AS crop_types
    FROM plots p
    JOIN users u ON u.id = p."userId"
    WHERE p.state     = 'growing'
      AND p."readyAt"    <= NOW()
      AND p."witheredAt"  > NOW()
      AND u."telegramId" IS NOT NULL
      AND u."isBanned"   = false
    GROUP BY u."telegramId", u.username
    HAVING COUNT(p.id) >= $1
...
async function notifyWitherWarning() {
  ...
  const { rows } = await pool.query(`
    SELECT
      ...
      ARRAY_AGG(DISTINCT p."cropType") AS crop_types,
      MIN(p."witheredAt")              AS soonest_wither
    FROM plots p
    JOIN users u ON u.id = p."userId"
    WHERE p.state        = 'growing'
      AND p."witheredAt" BETWEEN NOW() AND NOW() + ($1 || ' minutes')::interval
      AND u."telegramId" IS NOT NULL
      AND u."isBanned"   = false
    GROUP BY u."telegramId", u.username
  `, [warnMs]);
...
async function notifyDailyReward() {
  ...
  const { rows } = await pool.query(`
    ...
    WHERE u."telegramId" IS NOT NULL
      AND u."isBanned"   = false
      AND (
        u."lastDailyClaimAt" IS NULL
        OR u."lastDailyClaimAt" < NOW() - INTERVAL '24 hours'
      )
  `);
...
async function notifyEnergyFull() {
  ...
  const { rows } = await pool.query(`
    ...
    WHERE u."telegramId"   IS NOT NULL
      AND u."isBanned"      = false
      AND u.energy          < u."maxEnergy"
      AND u."energyRegenAt" IS NOT NULL
      AND u."energyRegenAt" <= NOW()
  `);
